  ...
  if (!markets || markets.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 dark:text-gray-400">Try searching for different terms</p>
      </div>
    )
  }
...
const MarketCard: React.FC<{ market: any }> = ({ market }) => {
  ...
  return (
    <div className="group bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 p-6 hover:shadow-3xl hover:scale-105 transition-all duration-300 cursor-pointer">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center space-x-2 mb-2">
            <div className={`w-3 h-3 rounded-full ${getStatusColor(market.active, market.closed)}`}></div>
      ...
      {market.category && (
        <div className="mb-4">
          <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r ${getCategoryColor(market.category)} text-white`}>
            {market.category}
          </div>
        </div>
      )}
      ...
      <div className="mt-4 pt-4 border-t border-gray-200/50 dark:border-gray-700/50">
        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>
            {market.commentCount || 0} comments
          </span>
        </div>
      </div>
    </div>
